...
- `setup.nix`: one-time activation that seeds `~/.gitconfig` from `external/gitconfig_defaults/{google,public}`, seeds `~/.config/mise/config.toml` from `template/mise-config.toml` on fresh installs, migrates the old Home Manager-managed `mise` symlink into a real file on existing systems, and writes `~/.setup-done`.
