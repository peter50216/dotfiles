...
- Put shared Nix core package/config changes in `packages.nix`; keep machine-specific additions in `local.nix`.
- Put shared `mise` defaults and baseline tools in `external/mise/00-dotfiles.toml`, not Home Manager `programs.mise.globalConfig`.
